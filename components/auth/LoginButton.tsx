...
export function LoginButton({ nextPath = "/", label = "Đăng nhập bằng Google" }: { nextPath?: string; label?: string }) {
  ...
  return (
    <button className="google-login-button" onClick={() => void login()} disabled={pending}>
      {pending ? <LoaderCircle className="spin" size={21} /> : <GoogleIcon />}
      <span>{pending ? "Đang chuyển đến Google…" : label}</span>
    </button>
  );
}
