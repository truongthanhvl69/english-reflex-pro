...
import { ArrowLeft, Bot, Check, ChevronDown, FileAudio, FileUp, GraduationCap, LayoutDashboard, MoreHorizontal, Pencil, Plus, Search, Send, Settings, Sparkles, Trash2, Users, X, Crown, CreditCard } from "lucide-react";
import { sentences } from "@/data/sentences";
import MembershipAdmin from "@/components/admin/MembershipAdmin";
import PaymentOrdersAdmin from "@/components/admin/PaymentOrdersAdmin";
import PaymentSettingsAdmin from "@/components/admin/PaymentSettingsAdmin";
import { useAuth } from "@/hooks/useAuth";

const sections = ["Tổng quan", "Khóa học", "Danh mục", "Bài học", "Câu luyện", "Từ vựng", "Audio", "Người dùng", "Thành viên", "Đơn hàng", "Cấu hình thanh toán", "Xếp hạng"];
...
export function AdminConsole() {
  ...
  // Unified Admin state loader
  const [adminData, setAdminData] = useState<any>({ members: [], payments: [], orders: [], settings: {} });
  ...
  useEffect(() => {
    if (section === "Thành viên" || section === "Đơn hàng" || section === "Cấu hình thanh toán") {
      void fetchAdminData();
    }
  }, [section]);
    ...
    <div className="admin-shell">
      <aside className="admin-sidebar">
        <div className="admin-brand">
          ...
          <div>
            <strong>Reflex Admin</strong>
            <small>CONTENT STUDIO</small>
          </div>
        </div>
        <nav style={{ overflowY: "auto", maxHeight: "calc(100vh - 160px)" }}>
          {sections.map((item) => {
            const getIcon = () => {
              if (item === "Tổng quan") return <LayoutDashboard size={17} />;
              if (item === "Người dùng") return <Users size={17} />;
              if (item === "Thành viên") return <Crown size={17} />;
              if (item === "Đơn hàng") return <CreditCard size={17} />;
              if (item === "Cấu hình thanh toán") return <Settings size={17} />;
        ...
        <div className="admin-sidebar-bottom">
          <button onClick={() => setSection("Cấu hình thanh toán")}><Settings size={17} /> Cài đặt hệ thống</button>
          <a href="/"><ArrowLeft size={17} /> Về app học</a>
        </div>
      </aside>
      <main className="admin-main">
        ...
        {section === "Thành viên" ? (
          ...
          <div style={{ padding: "0 0 40px" }}>
            {loadingAdmin ? (
              <div style={{ padding: "40px", textAlign: "center", color: "var(--muted)" }}>Đang tải dữ liệu đơn hàng...</div>
            ) : (
              <PaymentOrdersAdmin 
        ...
        ) : section === "Cấu hình thanh toán" ? (
          <div style={{ padding: "0 0 40px" }}>
            {loadingAdmin ? (
              <div style={{ padding: "40px", textAlign: "center", color: "var(--muted)" }}>Đang tải cấu hình cổng...</div>
            ) : (
